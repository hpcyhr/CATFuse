#!/usr/bin/env python
"""
Phase C-3 — Train SpikingVGG11_bn on CIFAR10 with LIFNode.

Matches bench_vgg11_e2e.py::build_vgg_model:
  - spiking_vgg.spiking_vgg11_bn(spiking_neuron=neuron.LIFNode, ...)
  - surrogate_function=surrogate.Sigmoid()
  - detach_reset=True
  - functional.set_step_mode(model, 'm')

NO CIFAR10 stem adaption needed: VGG11 natively uses 3x3 convolutions
and has AdaptiveAvgPool before the classifier, so it accepts any spatial
input size including 32x32.

Usage:
    cd /data/dagongcheng/yhrtest/CATFuse
    nohup python phaseC3_train_vgg11_cifar10.py \
        --data-path data/cifar10 \
        --output-dir checkpoints/ \
        --epochs 200 -b 128 --T 4 --lr 0.1 --amp \
        --device cuda:1 \
        > logs/phaseC3_train_$(date +%Y%m%d_%H%M%S).log 2>&1 &
"""
import argparse
import datetime
import os
...
def build_vgg_model(num_classes=10, v_threshold=1.0, tau=2.0):
...
def main():
    ...
    print(f"=== Phase C-3: SpikingVGG11_bn on CIFAR10 (LIFNode) ===")
    ...
    net = build_vgg_model(num_classes=10, v_threshold=args.v_threshold,
                          tau=args.tau).to(device)
    n_params = sum(p.numel() for p in net.parameters() if p.requires_grad)
    print(f"Model: SpikingVGG11_bn LIFNode (tau={args.tau}, v_th={args.v_threshold}), "
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > best_acc:
            ...
            torch.save({
                ...
                'test_acc': test_acc,
                'args': vars(args),
                'model_name': 'spiking_vgg11_bn',
            }, Path(args.output_dir) / 'spiking_vgg11_cifar10_lif_best.pth')
    ...
    torch.save({
        ...
        'args': vars(args),
        'model_name': 'spiking_vgg11_bn',
    }, Path(args.output_dir) / 'spiking_vgg11_cifar10_lif_final.pth')
