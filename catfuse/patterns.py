...
class FusedLinearLIF(CTFPattern):
    ...
    def _forward_impl(self, x):
        T, B = x.shape[0], x.shape[1]
        # 1. BatchFold linear
        x_2d = x.reshape(T * B, -1)
        # weight_io may be [out, in] or [in, out] depending on construction
        w = self.weight_io
        if w.shape[0] != x_2d.shape[1] and w.shape[1] == x_2d.shape[1]:
            # Standard [out, in] layout
            z_2d = F.linear(x_2d, w)
        else:
...
# ============================================================
# CTFSEWBottleneck — for SEW-ResNet50/101/152 Bottleneck replacement
# ============================================================
#
# Append this file's content to catfuse_patterns.py immediately BEFORE
# the `PATTERN_REGISTRY = { ... }` block (around line 758), then update
# PATTERN_REGISTRY and __all__ to include the two new classes.

class CTFSEWBottleneck(CTFPattern):
    """
    CTF-fused replacement for SEW-ResNet50/101/152's Bottleneck.

    Original SJ SEW Bottleneck.forward:
        identity = x
        out = conv1(x); bn1; sn1      # 1x1 reduce
        out = conv2(out); bn2; sn2    # 3x3 main
        out = conv3(out); bn3; sn3    # 1x1 expand
        if downsample:
            identity = downsample_sn(downsample(x))
        out = sew_function(identity, out, cnf)
        return out

    All three sn* sites are preceded by Conv+BN, so all three are valid
    PartialFusionConvBNLIF patterns. Downsample + downsample_sn is the
    fourth such pattern (optional). The sew_function ADD is post-LIF
    spike-level, handled inline.
    """

    def __init__(self, fused_first, fused_middle, fused_last,
                 fused_downsample, cnf):
        super().__init__()
        self.fused_first = fused_first      # conv1(1x1) + bn1 + sn1
        self.fused_middle = fused_middle    # conv2(3x3) + bn2 + sn2
        self.fused_last = fused_last        # conv3(1x1) + bn3 + sn3
        self.fused_downsample = fused_downsample  # may be None
        self.cnf = cnf

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = self.fused_first(x)
        out = self.fused_middle(out)
        out = self.fused_last(out)
        if self.fused_downsample is not None:
            identity = self.fused_downsample(x)
        else:
            identity = x
    ...
    @classmethod
    def from_sj_block(cls, sj_block):
        """Build from an SJ SEW-ResNet Bottleneck."""
        fused_first = PartialFusionConvBNLIF.from_sj_modules(
            sj_block.conv1, sj_block.bn1, sj_block.sn1
        )
        fused_middle = PartialFusionConvBNLIF.from_sj_modules(
            sj_block.conv2, sj_block.bn2, sj_block.sn2
        )
        fused_last = PartialFusionConvBNLIF.from_sj_modules(
            sj_block.conv3, sj_block.bn3, sj_block.sn3
        )
        fused_downsample = None
        if sj_block.downsample is not None:
            ds_conv = sj_block.downsample[0]
        ...
        return cls(fused_first, fused_middle, fused_last,
                   fused_downsample, sj_block.cnf)


# ============================================================
# CTFSpikingBottleneck — for standard spiking_resnet50/101/152 Bottleneck
# ============================================================

class CTFSpikingBottleneck(CTFPattern):
    """
    CTF-fused replacement for standard spiking_resnet Bottleneck.

    Original SJ spiking_resnet Bottleneck.forward:
        identity = x
        out = conv1(x); bn1; sn1      # 1x1, LIF after
        out = conv2(out); bn2; sn2    # 3x3, LIF after
        out = conv3(out); bn3          # 1x1, NO LIF yet
        if downsample: identity = downsample(x)  # Conv+BN, NO LIF
        out = out + identity           # Add BEFORE LIF
        out = sn3(out)                 # LIF LAST
        return out

    Pattern decomposition:
      - conv1+bn1+sn1       = PartialFusionConvBNLIF
      - conv2+bn2+sn2       = PartialFusionConvBNLIF
      - conv3+bn3+(+id)+sn3 = PartialFusionConvBNAddLIF   (Add before LIF)
      - downsample (opt)    = inline Conv+BN fold (no LIF)

    Difference from CTFSpikingBasicBlock:
      - adds a middle PartialFusionConvBNLIF (the 3x3 conv)
    """

    def __init__(self, fused_first, fused_middle, fused_last_add,
                 downsample_conv_weight, downsample_bn_scale, downsample_bn_bias,
                 downsample_stride, downsample_padding):
        super().__init__()
        self.fused_first = fused_first        # conv1(1x1) + bn1 + sn1
        self.fused_middle = fused_middle      # conv2(3x3) + bn2 + sn2
        self.fused_last_add = fused_last_add  # conv3(1x1) + bn3 + Add + sn3
        if downsample_conv_weight is not None:
            self.has_downsample = True
            self.downsample_stride = downsample_stride
            self.downsample_padding = downsample_padding
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.has_downsample:
            ...
            identity = x
        # conv3 + bn3 + Add(identity) + sn3 in one fused call
        out = self.fused_last_add(out, identity)
        return out

    @classmethod
    def from_sj_block(cls, sj_block):
        """Build from an SJ spiking_resnet Bottleneck."""
        fused_first = PartialFusionConvBNLIF.from_sj_modules(
            sj_block.conv1, sj_block.bn1, sj_block.sn1
        )
        fused_middle = PartialFusionConvBNLIF.from_sj_modules(
        ...
        fused_last_add = PartialFusionConvBNAddLIF.from_sj_modules(
            sj_block.conv3, sj_block.bn3, sj_block.sn3
        )
        if sj_block.downsample is not None:
            ds_conv = sj_block.downsample[0]
            ...
            ds_padding = ds_padding[0] if isinstance(ds_padding, tuple) else ds_padding
            return cls(
                fused_first, fused_middle, fused_last_add,
                downsample_conv_weight=ds_conv.weight.data,
                downsample_bn_scale=ds_scale.detach(),
                downsample_bn_bias=ds_bias.detach(),
                downsample_stride=ds_stride,
            ...
            return cls(fused_first, fused_middle, fused_last_add,
                       None, None, None, 0, 0)
...
PATTERN_REGISTRY = {
    ...
    'CTFSEWBottleneck':          CTFSEWBottleneck,          # 新
    'CTFSpikingBottleneck':      CTFSpikingBottleneck,      # 新
}

# Import CATFuse-SF spatio-temporal pattern (SparseFlow spatial backend)
try:
    from catfuse.sparseflow.ops.st_fusion_conv_bn_lif import STFusionConvBNLIF
    PATTERN_REGISTRY['STFusionConvBNLIF'] = STFusionConvBNLIF
except ImportError:
    STFusionConvBNLIF = None
...
__all__ = [
    ...
    'CTFSEWBottleneck',           # 新
    'CTFSpikingBottleneck',       # 新
    'STFusionConvBNLIF',          # CATFuse-SF
    'PATTERN_REGISTRY',
]